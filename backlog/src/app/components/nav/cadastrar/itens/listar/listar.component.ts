...
export class ListarComponent implements OnInit {
  ...
  creatForm(f: structItens) {
    this.editFormItens = new FormGroup({
      'servico': new FormControl(f.servico, [Validators.required]),//Validators.required coloca campo como obrigatorio
      'quantidade': new FormControl(f.quantidade, [Validators.required]),
      'valor_unitario': new FormControl(f.valor_unitario, [Validators.required]),
      'valor_total': new FormControl(f.valor_total),
